...
from .forms import loginform, newsform, flatform, flat_search_form, newclientform, client_edit_form, doma_new_post, \
    ...
    adm_form, DmTextForm, vestum_count_form
...
def flat_postForm(request):
    n1='Квартиры'
    n2='подача на Cайт, RegionalRealty, ДомКлик(СберБанк) и Yandex недвижимость'
    n3 = zayavka.objects.filter(status='Свободен').count()
    if request.POST:
...
def MegaRealfeedview(request):
    ...
    date = datetime.now()
    post = feed.objects.filter(pub='Да').order_by('-date_sozd')
    gal = feed_gallery.objects.all()
...
def vestumfeedview(request):
    count = flat_obj.objects.all().exclude(kadastr='').count()#*0.1)*2
    post = flat_obj.objects.filter(vestum_pub='Да',type = 'flat').exclude(kadastr='').order_by('-pk')[:100]
    gal = flat_obj_gal.objects.all()
    #post = flat_obj.objects.filter(domclick_pub='Да').order_by('-datep')
    #post = flat_obj.objects.order_by('-datep')
   ...
   # dm = get_object_or_404(domclickText, day = int(date1))
    # end of autoручной ввод текста сео
    return render(request,'any/vestum.html',{'tppost': post, 'tpgal':gal, 'tdate':date, 'tcount':count, }, content_type="text/xml")
...
def reeelt_otchet_all_view(request):
    ...
    if request.user.groups.get().name == 'Администрация':
        ...
        razn_otch =str(otchet_nov.objects.filter(date_zakr__gte=ds,date_zakr__lte=de, sdelka_zakrita='Да', ot_kuda_kl='Другое').count())
        Avito =str(otchet_nov.objects.filter(date_zakr__gte=ds,date_zakr__lte=de, sdelka_zakrita='Да', ot_kuda_kl='Avito').count())
        AvitoTurbo =str(otchet_nov.objects.filter(date_zakr__gte=ds,date_zakr__lte=de,  sdelka_zakrita='Да', ot_kuda_kl='Avito Turbo').count())
        Cian =str(otchet_nov.objects.filter(date_zakr__gte=ds,date_zakr__lte=de,  sdelka_zakrita='Да', ot_kuda_kl='Cian').count())
        sait =str(otchet_nov.objects.filter(date_zakr__gte=ds,date_zakr__lte=de,  sdelka_zakrita='Да', ot_kuda_kl='Сайт компании').count())
        rec =str(otchet_nov.objects.filter(date_zakr__gte=ds,date_zakr__lte=de, sdelka_zakrita='Да', ot_kuda_kl='По рекомендации').count())
        domclick = str(otchet_nov.objects.filter(date_zakr__gte=ds, date_zakr__lte=de, sdelka_zakrita='Да',
                                              ot_kuda_kl='Домклик(Сбер)').count())
        Yandex = str(otchet_nov.objects.filter(date_zakr__gte=ds, date_zakr__lte=de, sdelka_zakrita='Да',
...
def reelt_sdelka_otchet_detail_view(request, idd):
    ...
    return render(request, 'any/reelt_otchet_detail.html', {'tsdelka': sdelka, 'tn1': n1, 'tn2': n2, 'tn3':n3, 'tn11': n11, 'tn22': n22, 'tn33': n33, 'tn44': n44, 'tn55': n55, 'tn66': n66, 'tn77': n77, 'tn88': n88, 'tn99': n99, 'tn10': n10,
                                                            'tpn11':p11, 'tpn22':p22, 'tpn33':p33, 'tpn44':p44, 'tpn55':p55,'tpn66':p66,'tpn77':p77,'tpn11':p11,
                                                            'tpn88': p88,'tpn99':p99,'tpn10':p10,'t_my_ya_obj':my_ya_obj,'tcrm_obj_week_count':crm_obj_week_count, })
...
def otchet_edit_view(request, idd):
    ...
    if request.POST:
        ...
        if otchet.sdelka_zakrita=='Рассрочка':
            ...
            rasr_pr = 'No_rasr'
    return render(request, 'any/newotchet.html',{'tpotchformall':form, 'tsave':save, 'tgroup':group, 'tid':id, 'tRasrPr':rasr_pr})
...
def stat_count_crm_obj(request):
    ...
    return render(request,'crm/stat/crm_obj_index.html',
                   ...
                   'tOtd4_cn': otdel4_cn, 'tOtdAdl_cn': AdlOtdel_cn,'tall':all,
                     't_my_ya_obj': my_ya_obj,'tNach_otd':nach_otd, 'tegr':egr,
...
def stat_count_crm_obj_past(request):
    ...
    otdel3 = stat_obj_crm.objects.filter(auth_group='3 Отдел').order_by('-crm_calc')
    otdel4 = stat_obj_crm.objects.filter(auth_group='4 Отдел').order_by('-crm_calc')
    AdlOtdel = stat_obj_crm.objects.filter(auth_group='Офис в Адлере').order_by(('-crm_calc'))
...
############################################################
    ...
    all_reelt = User.objects.filter(is_active=True).order_by('last_name')
############################################################
## End of Vetsum
############################################################
    if request.POST:
